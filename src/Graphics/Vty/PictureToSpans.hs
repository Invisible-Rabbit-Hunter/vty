...
import Data.Monoid.SemiDirectProduct.Strict (Semi, embed, inject, tag, unSemi)
import Data.STRef
import qualified Data.Text.Lazy as TL
import qualified Data.Text.Lazy.IO as TL
import qualified Data.Vector as Vector hiding (replicate, take)
import Data.Vector.Mutable (MVector (..))
import qualified Data.Vector.Mutable as MVector
import Debug.Trace (traceShowId)
import Graphics.Vty.Attributes (Attr, currentAttr, defAttr)
import Graphics.Vty.Image
import Graphics.Vty.Image.BBox (BBox (..), inside)
import Graphics.Vty.Image.Crop
...
data BlitEnv s = BlitEnv
  { _region :: BBox Int,
    -- | length _mrowOps == bboxHeight _region
    _mrowOps :: MRowOps s
  }

region :: Lens' (BlitEnv s) (BBox Int)
region = lens _region (\e r -> e {_region = r})

mrowOps :: Lens' (BlitEnv s) (MRowOps s)
mrowOps = lens _mrowOps (\e r -> e {_mrowOps = r})

newtype BlitM s a = BlitM {runBlitM :: BlitEnv s -> ST s a}
  deriving (Functor, Applicative, Monad, MonadReader (BlitEnv s)) via ReaderT (BlitEnv s) (ST s)

newtype Blit s = Blit {getBlit :: Attr -> Semi (Crop Int) (Translation Int) -> BlitM s ()}

instance Semigroup (Blit s) where
  Blit f <> Blit g = Blit \a p -> f a p *> g a p

instance Monoid (Blit s) where
  mempty = Blit \_ _ -> pure ()

instance Action (Translation Int) (Blit s) where
  act t (Blit s) = Blit \a p -> s a (p <> embed t)

instance Action (Crop Int) (Blit s) where
  act c (Blit s) = Blit \a p -> s a (p <> inject c)

instance Action Attr (Blit s) where
  act a (Blit s) = Blit \a' p -> s (a <> a') p

instance PrimMonad (BlitM s) where
  type PrimState (BlitM s) = s
  primitive a = BlitM \_ -> primitive a

blitPrim :: Prim -> Blit s
blitPrim prim = Blit \a' p -> do
  let (c, t) = unSemi p
  rowOps <- asks _mrowOps
  reg <- asks _region
  case act (Crop reg <> c) $ act t (Just prim) of
    Nothing -> pure ()
    Just (SpanAt a x y str dispWidth charWidth) -> do
      ops <- MVector.read rowOps y
      let (opsL, ops') = splitOpsAt x ops
      let (_, opsR) = splitOpsAt dispWidth ops'
      MVector.write
        rowOps
        y
        ( Vector.concat
            [ opsL,
              Vector.singleton
                ( TextSpan
                    { textSpanAttr = a <> a',
                      textSpanOutputWidth = dispWidth,
                      textSpanCharWidth = charWidth,
                      textSpanText = str
                    }
                ),
              opsR
    ...
    Just (BGFill r') -> do
      let x = bboxLeft r'
      let width = bboxRight r' - x
      for_ [bboxTop r' .. bboxBottom r' - 1] \y -> do
        ops <- MVector.read rowOps y
        let (opsL, ops') = splitOpsAt (bboxLeft r') ops
        let (_, opsR) = splitOpsAt width ops'
        MVector.write rowOps y (Vector.concat [opsL, Vector.singleton (Skip width), opsR])
...
displayOpsForPic pic r =
  Vector.create do
    if regionWidth r == 0 || regionHeight r == 0
      then MVector.new 0
      else do
        let reg = BBox 0 (regionHeight r) 0 (regionWidth r)
        rowOps <- MVector.replicate (regionHeight r) (Vector.singleton (Skip (regionWidth r)))
        (`runBlitM` BlitEnv reg rowOps) do
          getBlit (foldImage blitPrim (picImage pic)) defAttr mempty
        substituteSkips (picBackground pic) rowOps
...
substituteSkips (Background {backgroundChar, backgroundAttr}) ops = do
  ...
  case safeWcwidth backgroundChar of
    w
      | w == 0 -> error $ "invalid background character " ++ show backgroundChar
      | w == 1 -> do
          forM_ [0 .. MVector.length ops - 1] $ \row -> do
            rowOps <- MVector.read ops row
...
swapSkipsForSingleColumnCharSpan c a = Vector.map f
  where
    f (Skip ow) =
      let txt = TL.take (toEnum ow) $ TL.repeat c
...
printPicture :: DisplayRegion -> Image -> IO ()
printPicture bds img = mapM_ (mapM_ TL.putStr) $ fmap (fmap (\s -> case s of TextSpan {} -> textSpanText s; _ -> "\n")) (displayOpsForPic (picForImage img) bds)
...
--       then
--         let textStr' = clipText textStr leftClip rightClip
...
--   case state ^. skip of
--     s
--       | s > size -> put $ state & skip %~ subtract size
--       | s == 0 ->
--           if state ^. remaining > i0Dim
--             then do
--               addMaybeClipped i0
...
--       error $
--         "row " ++ show row ++ " now exceeds region width"
