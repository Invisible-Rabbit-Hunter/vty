...
import Graphics.Vty.Image.Translation (translate, translateX, translateY)
...
backgroundFill ::
  BBox Int ->
  Image
backgroundFill bbox = Prim (BGFill bbox)

-- | Combines two images horizontally. This is an alias for 'horizJoin'.
beside :: Image -> Image -> Image
beside d1 d2 =
  case (getBBox d1, getBBox d2) of
    (Just bbox1, Just bbox2) ->
      let dx = bboxRight bbox1 - bboxLeft bbox2
       in d1 <> translate dx 0 d2
    _ -> d1 <> d2
...
above d1 d2 =
  case (getBBox d1, getBBox d2) of
    (Just bbox1, Just bbox2) -> let dy = bboxBottom bbox1 - bboxTop bbox2 in d1 <> translate 0 dy d2
    _ -> d1 <> d2
...
text :: TL.Text -> Image
text txt =
  let displayWidth = safeWctlwidth txt
   in Prim (SpanAt mempty 0 0 txt displayWidth (fromIntegral $! TL.length txt))

-- | Make an 'Image' from text value. The text value should be
-- sanitized of escape sequences (ASCII 27) and carriage returns;
-- otherwise layout and attribute problems may result.
text' :: T.Text -> Image
text' txt =
  let displayWidth = safeWctwidth txt
   in Prim (SpanAt mempty 0 0 (TL.fromStrict txt) displayWidth (T.length txt))

-- | Make an image from single character. This is a standard Haskell
-- 31-bit character assumed to be in the ISO-10646 encoding.
char :: Char -> Image
char c =
  let displayWidth = safeWcwidth c
   in Prim (SpanAt mempty 0 0 (TL.singleton c) displayWidth 1)
...
iso10646String :: String -> Image
iso10646String str =
  let displayWidth = safeWcswidth str
   in Prim (SpanAt mempty 0 0 (TL.pack str) displayWidth (length str))
...
utf8String :: [Word8] -> Image
utf8String bytes = utf8Bytestring (BL.pack bytes)

-- | Make an 'Image' from a UTF-8 encoded lazy bytestring.
utf8Bytestring :: BL.ByteString -> Image
utf8Bytestring bs = text (TL.decodeUtf8 bs)

-- | Make an 'Image' from a UTF-8 encoded strict bytestring.
utf8Bytestring' :: B.ByteString -> Image
utf8Bytestring' bs = text' (T.decodeUtf8 bs)
  ...
  | otherwise =
      vertCat $
        replicate (fromIntegral h) $
          Prim $
            SpanAt a 0 0 txt displayWidth charWidth
  where
...
pad inL inT inR inB inImage
  ...
  | otherwise =
      case getBBox inImage of
        Just BBox {..} ->
          backgroundFill (BBox (bboxTop - inT) (bboxBottom + inB) (bboxLeft - inL) (bboxRight + inR)) <> inImage
        Nothing -> inImage
